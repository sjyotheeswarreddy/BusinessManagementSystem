...
$sql = mysqli_query($con, "SELECT * FROM merchant WHERE id='$id'");
...
$getdate='';
$mysqlt = mysqli_query($con, "SELECT * FROM quotation WHERE quotation_id='".$qid."' and user_id=".$id."");
while ($myrowt = mysqli_fetch_array($mysqlt)) {
    $cusname = $myrowt["customername"];
    $cusadd = $myrowt["customeraddress"];
	$getdate = $myrowt["submitdate"];
}
...
<body>
<h2 align="center">Quotation Invoice</h2>
<div class="container" style="border: 2px solid black;">
    ...
    <table class="header" style="width:100%; border-bottom: 1px solid black;">
        <tbody>
            ...
            <tr>
                <th align="right" style="color:black;">'.$username.'</th>
            </tr>
            <tr>
                <th align="right" style="color:black;">'.$address.'</th>
            </tr>
        </tbody>
    </table>
    </header>
    ...
    <table class="header" align="right">
        <tbody>
        <tr bgcolor="purple">
                <th align="right">Quotation ID.: quid'.$qid.'</th>
            </tr>
            <tr>
    ...
    <table width="100%" cellspacing="10" style="border: 1px solid black; border-collapse: collapse; text-align: center;">
        <tbody>
            ...
            $mysql = mysqli_query($con, "SELECT * FROM quotation WHERE quotation_id='".$qid."' and user_id=".$id."");
            $i=0;
            $totalgst = 0;
            $quty = 0;
            $amt = 0;
