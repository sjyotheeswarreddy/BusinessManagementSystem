...
$query = mysqli_query($con, "SELECT distinct(sale_id) FROM sale WHERE user_id='$manid'");
$i = 1;
while ($row = mysqli_fetch_array($query)) {
  $sqlquery = mysqli_query($con, "SELECT * FROM sale WHERE sale_id='".$row["sale_id"]."' AND user_id='$manid'");
  $cusname = "";
  $prod = 0;
  $quty = 0;
  $price = 0;
  ...
  while ($sqlrow = mysqli_fetch_array($sqlquery)) {
    $cusname=$sqlrow['vendor_name'];
    $prod=$prod+1;
    $date=$sqlrow['submitdate'];
    $quty+=$sqlrow['qty'];
    $price=$sqlrow['grand_total'];
...
<td>Purchase_<?php echo $row['sale_id'] ?></td>
<td><?php echo $prod ?></td>
<td><?php echo $quty ?></td>
<td><?php echo $price ?></td>
<!-- <td><?php if ($status == 'Accepted') {?>
   <a href="view_pdf.php?id=<?php echo $userid; ?>&qid=<?php echo $row['sale_id']; ?>&mode=view">View</a>
   <br>
...
<td>
  <a href="purchase_view.php?file=<?php echo $bill ; ?>"><button type="button" class="btn btn-info pb-0 pt-0 ps-1 pe-1">Download</button></a>
</td>
</tr>
<?php $i++;} ?>
