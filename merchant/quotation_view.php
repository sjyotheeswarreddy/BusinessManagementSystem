...
$admin = $_SESSION['MERCHANT_LOGIN'];

$qw = mysqli_query($con, "select * from  merchant where id='$admin'");
                            ...
                            <li class="user-profile header-notification">
                                ...
                                <ul class="show-notification profile-notification">
                                    ...
                                    <li class="waves-effect waves-light">
                                      ...
                                      <li class="waves-effect waves-light">
                                        <a href="subscriptiondetails">
                                            <i class="ti-settings"></i> Subscripton
                                        </a>
                                    </li> 
                                    <li class="waves-effect waves-light">
                                        <a href="logout.php">
                                ...
                                <div class="row align-items-center">
                                    <div class="col-md-8">
                                        <div class="page-header-title">
                                            <h5 class="m-b-10">Quotations View</h5>
                                            ...
                                            <div class="card-block table-border-style">
                                                <div class="table-responsive">
                                                    ...
                                                    <table class="table table-hover table-bordered">
                                                        ...
                                                        <?php 
include('config.php');
$query = mysqli_query($con, "SELECT distinct(quotation_id) FROM quotation WHERE user_id='$userid'");
$i = 1;
while ($row = mysqli_fetch_array($query)) {
    $quotation_id=$row["quotation_id"];
  $sqlquery = mysqli_query($con, "SELECT * from quotation where quotation_id='$quotation_id' and user_id='$userid'");
  $cusname = "";
  $prod = 0;
  $quty = 0;
  $price = 0;
  ...
  while ($sqlrow = mysqli_fetch_array($sqlquery)) {
    $cusname=$sqlrow['customername'];
    $prod=$prod+1;
    $date=$sqlrow['submitdate'];
    $quty+=$sqlrow['quantity'];
    $price+=$sqlrow['grandtotal'];
...
<td>quid<?php echo $row['quotation_id'] ?></td>
<td><?php echo $prod ?></td>
<td><?php echo $quty ?></td>
<td><?php echo $price ?></td>
<td>
   <a href="view_pdf.php?id=<?php echo $userid; ?>&qid=<?php echo $row['quotation_id']; ?>&mode=view"><button type="button" class="btn btn-info pt-0 ps-0 pb-1 pe-1">View </button></a>
   <br>
  </td> 
<td><a href="view_pdf.php?id=<?php echo $userid; ?>&qid=<?php echo $row['quotation_id']; ?>&mode=download"><button type="button" class="btn btn-info pt-0 ps-0 pb-1 pe-1">Download</button></a></td>


<!-- <td>
 <select onchange="window.location='status_check.php?id='+this.value;" id="quotstatus">
  <option></option>
  <option <?php //if($status == 'Accepted') {?> selected="selected"<?php //} ?> value="<?php //echo $row['quotation_id'] ?>,Accepted">Accepted</option>
  <option <?php //if($status == 'Rejected') {?> selected="selected"<?php //} ?> value="<?php //echo $row['quotation_id'] ?>,Rejected">Rejected</option>
  <option <?php //if($status == 'Pending') {?> selected="selected"<?php //} ?> value="<?php //echo $row['quotation_id'] ?>,Pending">Pending</option>   
 </select>
</td> -->
<td>
 <select id="quotstatus<?php echo $i ?>" class="selectStatus">
  <option></option>
  <option <?php if($status == 'Accepted') {?> selected="selected"<?php } ?> value="<?php echo $row['quotation_id'] ?>,Accepted">Accepted</option>
  <option <?php if($status == 'Rejected') {?> selected="selected"<?php } ?> value="<?php echo $row['quotation_id'] ?>,Rejected">Rejected</option>
  <option <?php if($status == 'Pending') {?> selected="selected"<?php } ?> value="<?php echo $row['quotation_id'] ?>,Pending">Pending</option>   
...
<?php $i++;} ?>
  ...
  $('#fid_table').find('tr').each(function() {
   ...
   /* var trSel = $("#fid_table tr:not('.no-records'):visible");
            // Check for number of rows & append no records found row
            if (trSel.length == 0) {
                $("#fid_table").html('<tr class="no-records"><td>No record found.</td></tr>');
            }
            else {
                $('.no-records').remove();
            }*/
...
$('.selectStatus').on('change', function(){
  ...
  value = $("#"+value).val();
  //alert(value);
  window.location='status_check.php?id='+value;
});
